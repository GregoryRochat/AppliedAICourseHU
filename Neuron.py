...
class Neuron:
    ...
    def updateweights(self, outputerrors, stepsize):
        for i in range(0, len(outputerrors)):
            ...
            if self.bias:
                print(self.weights[i], stepsize, self.activation)
...
class NeuronLayer:
    ...
    def calculateerrors(self, expectedoutputlist, errorlist):
        if len(expectedoutputlist) != 0:
            ...
            for neuron in self.neurons:
                if not neuron.bias:
                    neuron.error = (1 - math.tanh(neuron.activation)) * sum(w*e for w, e in zip(neuron.weights, errorlist))
